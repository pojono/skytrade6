#!/usr/bin/env python3
"""
v42bb: Hurst Exponent Proxy + Price Acceleration + Return IQR + High-Low Position

EXP NNNN5: Hurst Exponent Proxy (Rescaled Range)
  - H > 0.5 = trending, H < 0.5 = mean-reverting
  - Extreme low H = strong MR → trade with MR
  - Extreme high H = strong trend → fade exhaustion

EXP OOOO5: Price Acceleration (2nd derivative)
  - Acceleration = velocity change = mom(N) - mom(N).shift(N)
  - Extreme acceleration → overextended → fade
  - Deceleration after acceleration → reversal

EXP PPPP5: Return IQR (Interquartile Range)
  - IQR = P75 - P25 of rolling returns
  - High IQR = volatile = more MR opportunity
  - Combined with direction for fade

EXP QQQQ5: High-Low Position in Range
  - Position = (close - low) / (high - low) over N bars
  - Near 1 = at top of range → fade
  - Near 0 = at bottom of range → fade
...
def hurst_proxy(series, window):
    """Fast Hurst exponent proxy using variance ratio."""
    ret = series.pct_change()
    var1 = ret.rolling(window, min_periods=window).var()
    # Variance of N-period returns vs N * variance of 1-period returns
    retN = series.pct_change(5)
    varN = retN.rolling(window, min_periods=window).var()
    # H = log(varN/var1) / (2*log(5)) + 0.5 approximately
    ratio = varN / (5 * var1 + 1e-20)
    h = np.log(ratio + 1e-10) / (2 * np.log(5)) + 0.5
    return h


def main():
    out_file = 'results/v42bb_hurst_accel2.txt'
    ...
    print("="*80)
    print(f"  v42bb: HURST PROXY + PRICE ACCEL + RETURN IQR + HL POSITION")
    ...
    for symbol in symbols:
        ...
        # ── EXP NNNN5: HURST EXPONENT PROXY ──
        print(f"\n  --- EXP NNNN5: HURST EXPONENT PROXY ---")

        for window in [30, 60]:
            h = hurst_proxy(close, window)
            h_pct = h.rolling(120, min_periods=60).rank(pct=True)

            # Low H = MR regime → fade with confidence
            for thresh in [0.10, 0.20]:
                low_h = h_pct < thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[low_h]:
                ...
                print(f"\n  Hurst({window}) <{thresh*100:.0f}th pct (MR fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'hurst_{window}_{thresh}')] = te_r

        # ── EXP OOOO5: PRICE ACCELERATION ──
        print(f"\n  --- EXP OOOO5: PRICE ACCELERATION ---")

        for period in [10, 20]:
            mom = close.pct_change(period)
            accel = mom - mom.shift(period)
            accel_pct = accel.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.05, 0.10]:
                accel_up = accel_pct > (1 - thresh)
                accel_down = accel_pct < thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[accel_up]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    t = sim_trade(bars, idx, False)  # fade upward acceleration
                    if t:
                        if ts < split_ts: train_trades.append(t)
                        else: test_trades.append(t)
                        lt = ts
                for ts in bars.index[accel_down]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    t = sim_trade(bars, idx, True)  # fade downward acceleration
                    if t:
                        if ts < split_ts: train_trades.append(t)
                        else: test_trades.append(t)
                        lt = ts
                ...
                print(f"\n  Accel({period}) <{thresh}/{1-thresh:.2f} (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'accel_{period}_{thresh}')] = te_r

        # ── EXP PPPP5: RETURN IQR ──
        print(f"\n  --- EXP PPPP5: RETURN IQR ---")

        for window in [30, 60]:
            p75 = ret_1m.rolling(window, min_periods=window).quantile(0.75)
            p25 = ret_1m.rolling(window, min_periods=window).quantile(0.25)
            iqr = p75 - p25
            ...
            for thresh in [0.90, 0.95]:
                ...
                print(f"\n  IQR({window}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
        ...
        # ── EXP QQQQ5: HIGH-LOW POSITION ──
        print(f"\n  --- EXP QQQQ5: HIGH-LOW POSITION ---")

        for window in [30, 60]:
            roll_high = high.rolling(window, min_periods=window).max()
            roll_low = low.rolling(window, min_periods=window).min()
            hl_pos = (close - roll_low) / (roll_high - roll_low + 1e-10)

            for thresh in [0.05, 0.10]:
                near_top = hl_pos > (1 - thresh)
                near_bot = hl_pos < thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[near_top]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                ...
                print(f"\n  HL pos({window}) <{thresh}/{1-thresh:.2f} (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'hlpos_{window}_{thresh}')] = te_r
