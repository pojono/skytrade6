...
if __name__ == '__main__':
    ...
    print("\n=== OPTIMIZED LONG-ONLY FLUSH RESULTS ===")
    ...
    print("\nExit Reasons:")
    print(df_port['reason'].value_counts())
    ...
    plt.title('Optimized Long-Only Liquidation Flush (Compounded)', fontsize=14)
    plt.xlabel('Date')
    plt.ylabel('Account Balance ($)')
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig('/home/ubuntu/Projects/skytrade6/gemini-2/equity_curve_optimized.png')
    
    # Save findings
    report = f"""# Final Strategy Findings: Optimized Liquidation Flush
...
- **Average Net Return per Trade:** {df_port['net_ret'].mean() * 10000:.2f} bps ({(df_port['net_ret'].mean() * 100):.2f}% per trade)
...
"""
    ...
    df_port.to_csv("/home/ubuntu/Projects/skytrade6/gemini-2/portfolio_trades_optimized.csv", index=False)
