#!/usr/bin/env python3
"""
v42bx: MACD Histogram Velocity + Channel Width Ratio + Return Sign Ratio + HL Position

EXP XXXX8: MACD Histogram Velocity
  - MACD = EMA(12) - EMA(26), signal = EMA(MACD, 9), hist = MACD - signal
  - hist_vel = hist - hist.shift(N)
  - Extreme hist velocity = rapid momentum shift → fade

EXP YYYY8: Channel Width Ratio (Bollinger bandwidth change)
  - bandwidth = (upper - lower) / ma
  - bw_ratio = bw / bw.shift(N)
  - High ratio = vol expanding rapidly → fade

EXP ZZZZ8: Return Sign Ratio (proportion of positive returns)
  - sign_ratio = rolling_mean(sign(ret))
  - Extreme sign ratio = persistent directional bias → fade

EXP AAAA9: High-Low Position (where close sits in recent range)
  - hl_pos = (close - rolling_low) / (rolling_high - rolling_low)
  - Extreme position (near 0 or 1) → fade
...
def main():
    out_file = 'results/v42bx_macd_chanwidth.txt'
    ...
    print("="*80)
    print(f"  v42bx: MACD HIST VEL + CHANNEL WIDTH + RET SIGN RATIO + HL POSITION")
    ...
    for symbol in symbols:
        ...
        for vel_lag in [3, 5]:
            ...
            for thresh in [0.90, 0.95]:
                extreme_hv = hv_pct > thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[extreme_hv]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    is_long = hist_vel.iloc[idx] < 0
                ...
                print(f"\n  MACD hist vel({vel_lag}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'mhv_{vel_lag}_{thresh}')] = te_r

        # ── EXP YYYY8: CHANNEL WIDTH RATIO ──
        print(f"\n  --- EXP YYYY8: CHANNEL WIDTH RATIO ---")

        for window in [20, 40]:
            ma = close.rolling(window, min_periods=window).mean()
            std = close.rolling(window, min_periods=window).std()
            bandwidth = (4 * std) / (ma + 1e-10)
            bw_ratio = bandwidth / bandwidth.shift(10)
            bwr_pct = bw_ratio.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.90, 0.95]:
                high_bwr = bwr_pct > thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[high_bwr]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                ...
                print(f"\n  Chan width({window}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'cw_{window}_{thresh}')] = te_r

        # ── EXP ZZZZ8: RETURN SIGN RATIO ──
        print(f"\n  --- EXP ZZZZ8: RETURN SIGN RATIO ---")

        ret_sign = np.sign(ret_1m)

        for window in [20, 40]:
            sign_ratio = ret_sign.rolling(window, min_periods=window).mean()
            sr_pct = sign_ratio.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.05, 0.10]:
                up_ext = sr_pct > (1 - thresh)  # persistent positive → fade short
                dn_ext = sr_pct < thresh  # persistent negative → fade long
                ...
                print(f"\n  Sign ratio({window}) <{thresh}/{1-thresh:.2f} (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'sr_{window}_{thresh}')] = te_r

        # ── EXP AAAA9: HIGH-LOW POSITION ──
        print(f"\n  --- EXP AAAA9: HIGH-LOW POSITION ---")
        ...
        for window in [30, 60]:
            ...
            for thresh in [0.05, 0.10]:
                near_high = hl_pos > (1 - thresh)  # near top → fade short
                near_low = hl_pos < thresh  # near bottom → fade long

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[near_high]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    t = sim_trade(bars, idx, False)
                    if t:
                ...
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'hl_{window}_{thresh}')] = te_r
