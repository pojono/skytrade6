#!/usr/bin/env python3
"""
v42az: Rolling Kurtosis + Price Channel Width + Close-Open Gap + Tail Ratio

EXP FFFF5: Rolling Return Kurtosis
  - High kurtosis = fat tails = extreme moves more likely
  - Extreme kurtosis + recent move → fade
  - Different from skewness: kurtosis is symmetric tail thickness

EXP GGGG5: Bollinger Band Width as Squeeze/Expansion
  - BB width = (upper - lower) / middle
  - Extreme narrow = squeeze → breakout coming
  - Extreme wide = expansion → mean reversion

EXP HHHH5: Close-to-Open Gap (Bar-to-Bar)
  - Gap = (open[t] - close[t-1]) / close[t-1]
  - Rolling sum of gaps as directional pressure
  - Extreme gap pressure → fade

EXP IIII5: Tail Ratio (Upper vs Lower Tail)
  - Upper tail = P95 of returns / median
  - Lower tail = P5 of returns / median
  - Ratio indicates asymmetric risk → fade dominant tail
...
def main():
    out_file = 'results/v42az_kurtosis_channel.txt'
    ...
    print("="*80)
    print(f"  v42az: KURTOSIS + BB WIDTH + CLOSE-OPEN GAP + TAIL RATIO")
    ...
    for symbol in symbols:
        ...
        # ── EXP FFFF5: ROLLING KURTOSIS ──
        print(f"\n  --- EXP FFFF5: ROLLING KURTOSIS ---")

        for window in [30, 60]:
            kurt = ret_1m.rolling(window, min_periods=window).kurt()
            kurt_pct = kurt.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.90, 0.95]:
                # High kurtosis + recent move → fade
                high_kurt = kurt_pct > thresh
        ...
        for window in [20, 40]:
            ...
            bb_width = (2 * std * 2) / (sma + 1e-10)  # 2-sigma band width
            bw_pct = bb_width.rolling(120, min_periods=60).rank(pct=True)

            # Extreme wide = expansion → fade
            for thresh in [0.90, 0.95]:
                wide = bw_pct > thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[wide]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    r = ret_1m.iloc[idx] if idx < len(ret_1m) else 0
                    is_long = r < 0
                ...
                print(f"\n  BB width({window}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'bbw_{window}_{thresh}')] = te_r

        # ── EXP HHHH5: CLOSE-OPEN GAP ──
        print(f"\n  --- EXP HHHH5: CLOSE-OPEN GAP ---")

        gap = (opn - close.shift(1)) / (close.shift(1) + 1e-10)

        for window in [10, 20]:
            cum_gap = gap.rolling(window, min_periods=window).sum()
            gap_pct = cum_gap.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.05, 0.10]:
                gap_up = gap_pct > (1 - thresh)
                gap_down = gap_pct < thresh
                ...
                for ts in bars.index[gap_up]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    t = sim_trade(bars, idx, False)  # fade gap up pressure
                    if t:
                        if ts < split_ts: train_trades.append(t)
                        else: test_trades.append(t)
                        lt = ts
                for ts in bars.index[gap_down]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    t = sim_trade(bars, idx, True)  # fade gap down pressure
                    if t:
                        if ts < split_ts: train_trades.append(t)
                        else: test_trades.append(t)
                        lt = ts
                ...
                print(f"\n  Gap pressure({window}) <{thresh}/{1-thresh:.2f} (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
        ...
        # ── EXP IIII5: TAIL RATIO ──
        print(f"\n  --- EXP IIII5: TAIL RATIO ---")

        for window in [30, 60]:
            p95 = ret_1m.rolling(window, min_periods=window).quantile(0.95)
            p05 = ret_1m.rolling(window, min_periods=window).quantile(0.05)
            tail_ratio = p95.abs() / (p05.abs() + 1e-10)
            tr_pct = tail_ratio.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.05, 0.10]:
                # High tail ratio = right tail dominant → fade up
                # Low tail ratio = left tail dominant → fade down
                high_tr = tr_pct > (1 - thresh)
                low_tr = tr_pct < thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[high_tr]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                ...
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'tail_{window}_{thresh}')] = te_r
