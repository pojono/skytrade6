...
# Generate parameter combinations without optimizing Chop (fixing it to strictly 61.8)
CONFIGS = []
for idx, (tp, sl, ker, donch, vol) in enumerate(itertools.product(
    ...
    [1.5, 2.0])):
    CONFIGS.append({
        'id': idx, 'tp': tp, 'sl': sl, 'ker': ker, 'donch': donch, 'vol': vol, 'chop_limit': 61.8
    })
...
def main():
    ...
    print(f"Generating WFO Matrix with FIXED Chop Filter (<=61.8)...")
    ...
    print("\n" + "="*85)
    print(f"--- WFO OOS RESULTS: FIXED CHOP FILTER <= 61.8 (Jan 2022 - Mar 2026) ---")
    print("="*85)
