...
for idx, (tp, sl, ker, donch, vol) in enumerate(itertools.product(
    [0.15, 0.20, 0.25], 
    [0.08, 0.10, 0.12], 
    [0.12, 0.15, 0.18], 
    [15, 20, 25], 
    [1.5, 2.0])):
    CONFIGS.append({
        'id': idx, 'tp': tp, 'sl': sl, 'ker': ker, 'donch': donch, 'vol': vol
    })
...
def process_symbol(args):
    ...
    if btc_regime is not None:
        ...
        df_4h['btc_ker'] = 1.0 # always trending if missing
...
def main():
    ...
    print(f"Generating Fast WFO Matrix for {len(CONFIGS)} configurations...")
    ...
    # Equity PnL = raw_pnl * (0.02 / SL_pct)
    ...
    while True:
        ...
        if len(df_train) == 0:
            best_config_id = 0 # Default fallback
            best_metric = 0
        else:
            # Aggregate fitness (Total Equity Return in Train Window)
            fitness = df_train.groupby('config_id')['equity_pnl'].sum()
            # Require at least 5 trades to avoid overfitting to 1 lucky trade
            trade_counts = df_train.groupby('config_id').size()
            valid_configs = fitness[trade_counts >= 5]
            
            if len(valid_configs) > 0:
                best_config_id = valid_configs.idxmax()
                best_metric = valid_configs.max()
            else:
                best_config_id = fitness.idxmax() if len(fitness) > 0 else 0
                best_metric = 0
                
        # 2. Test Phase (Strictly OOS)
        # Select trades matching best_config_id that *entered* during the test window
        ...
        wfo_results.append({
            'Train_Window': f"{current_train_start.strftime('%Y-%m')} to {train_end.strftime('%Y-%m')}",
            'Test_Window': f"{train_end.strftime('%Y-%m')} to {test_end.strftime('%Y-%m')}",
            'Best_Config': best_config_id,
            'Train_Return': best_metric,
            'OOS_Trades': len(test_period_trades),
            'OOS_Return': test_period_trades['equity_pnl'].sum() if not test_period_trades.empty else 0
        })
    ...
    print("\nWFO Phase Summaries:")
    for _, row in df_wfo.iterrows():
        cfg = CONFIGS[row['Best_Config']]
        cfg_str = f"TP:{cfg['tp']:.2f} SL:{cfg['sl']:.2f} KER:{cfg['ker']} DC:{cfg['donch']}"
        print(f"Train: {row['Train_Window']} | Test: {row['Test_Window']} | Best Cfg: [{cfg_str}] | OOS Ret: {row['OOS_Return']:.2%}")
        
    # Combine OOS Trades and run Portfolio Simulation
    ...
    final_oos_df['trade_pnl_usd'] = 0.0
    ...
    for idx, row in final_oos_df.iterrows():
        ...
        final_oos_df.at[idx, 'trade_pnl_usd'] = trade_usd_pnl
    ...
    plt.plot(dates, equity_curve, label='WFO OOS Portfolio Equity', color='green')
    plt.axhline(INITIAL_CAPITAL, color='red', linestyle='--', label='Starting Capital ($1000)')
    plt.title(f'STRICT OUT-OF-SAMPLE (WFO) - Asymmetric Macro Trend')
    plt.xlabel('Date')
    plt.ylabel('Account Balance ($)')
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    chart_path = '/home/ubuntu/Projects/skytrade6/gemini-1/wfo_oos_equity_curve.png'
    plt.savefig(chart_path)
    print(f"\nOOS Equity curve saved to {chart_path}")
    ...
    print("\n" + "="*85)
    print(f"--- STRICT WFO OUT-OF-SAMPLE RESULTS (Jan 2022 - Mar 2026) ---")
    print("="*85)
