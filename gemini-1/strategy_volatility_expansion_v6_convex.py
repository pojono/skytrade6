...
def backtest_strategy(df, symbol):
    # Convex strategy to push edge up
    # We found a positive expected edge of ~0.93% per trade with Sharpe 1.14 on V5
    # Let's adjust risk-reward to be more convex. 
    # Tighter SL = hit more often, but when it runs, let it run longer. 
    ...
    df['flush_zone'] = (df['ret_4h'] < -0.08) & (df['ret_4h'] > -0.50) & (df['oi_4h'] < -0.10)
    
    # Fill NAs and convert to boolean
    df['flush_recent'] = df['flush_zone'].rolling(24).max().fillna(0).astype(bool)
    
    # Confirmed Bounce -> require stronger confirmation: 15-min return > 3%
    df['ret_15m'] = df['close'] / df['close'].shift(3) - 1
    df['bounce_conf'] = df['ret_15m'] > 0.03
    ...
    SL_PCT = -0.06
    TP_PCT = 0.15 # Higher target
    TIME_STOP_CANDLES = 288 # 24H max hold
...
def main():
    ...
    print(f"Testing Confirmed Bounce V6 (3% Bounce + 15% TP + 24H Hold)...")
    ...
    print("\n=== CONFIRMED BOUNCE STRATEGY V6 RESULTS ===")
    ...
    trades_df.to_csv("/home/ubuntu/Projects/skytrade6/gemini-1/convex_liquidation_trades.csv", index=False)
    print("Saved trades to gemini-1/convex_liquidation_trades.csv")
