...
# We want something that generates consistent returns across all months.
# The most consistent, non-directional edge in crypto is capturing massive funding rate
# premiums when the market goes parabolic, and fading the resulting mean reversion.
# Let's write a purely statistical funding reversion strategy.
...
def load_symbol_data(symbol):
    ...
    # 4H candles to filter noise but capture enough trades
    ...
    if fund_list:
        ...
        df_fund = df_fund.resample('4h').ffill()
...
def process_symbol(args):
    ...
    # Funding usually stays near 0.01% per 8h. 
    # When it spikes (e.g. > 0.05%), it means severe leverage is built up.
    
    df['funding_mean_7d'] = df['fundingRate'].rolling(6 * 7).mean()
    df['funding_std_7d'] = df['fundingRate'].rolling(6 * 7).std()
    df['funding_z'] = (df['fundingRate'] - df['funding_mean_7d']) / df['funding_std_7d']
    
    # We look for a massive surge in BOTH funding (greed) and price (parabola)
    df['ret_24h'] = df['close'] / df['close'].shift(6) - 1
    
    # We want to SHORT when: Funding is > +3 Z-scores AND price is up > 10% in 24h
    df['short_signal'] = (df['funding_z'] > 3.0) & (df['ret_24h'] > 0.10)
    
    # We want to LONG when: Funding is < -3 Z-scores AND price is down > 10% in 24h
    df['long_signal'] = (df['funding_z'] < -3.0) & (df['ret_24h'] < -0.10)
    ...
    SL_PCT = 0.06 # 6% tight stop to cut losers quickly
...
def main():
    ...
    print(f"Testing High/Low Funding Z-Score Reversion...")
