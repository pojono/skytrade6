...
def process_symbol(args):
    ...
    # Use the mathematically optimal TP/SL found in v44
    TP_PCT = 0.20
    SL_PCT = 0.10
...
def main():
    # Only test on the 10 coins we downloaded deep history for
    symbols = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'DOGEUSDT', 'XRPUSDT', 'AVAXUSDT', 'LINKUSDT', 'ADAUSDT', 'DOTUSDT', 'NEARUSDT']
    
    print(f"Testing Optimal V43 on Deep History (Since Jan 2024)...")
    ...
    # 20% TP, 10% SL. Risking 2% per trade requires a 20% position size.
    # So Equity PnL = Raw PnL * 0.20
    df_trades['equity_pnl'] = df_trades['pnl'] * 0.20
    ...
    print("\n" + "="*80)
    print("--- DEEP HISTORY ASYMMETRIC TREND (TP 20% / SL 10%) ---")
    print("="*80)
    ...
    print(f"TOTAL PORTFOLIO RETURN (14 Months): {total_equity_return:.2%}")
