...
# The issue with v18 was relying on Taker exits for Stop Losses, which bleed the account.
# The issue with v19 was the setup wasn't strict enough, generating 3900 trades.
# Let's perfectly replicate the successful elements of Config 2 (No Stop Loss, Limit Entries/Exits, Short Holding Time)
# BUT we will add a catastrophic time-based stop.
...
def process_symbol(args):
    ...
    # STRICT CASCADE DETECTOR
    df['vol_ma'] = df['volume'].rolling(60).mean()
    # P95 volume roughly equals > 3x the 1h average
    df['vol_spike'] = df['volume'] > (df['vol_ma'] * 4.0)
    
    # Cascade down: Open to Low drop > 0.4% in 1 minute
    df['drop_pct'] = (df['low'] - df['open']) / df['open']
    df['cascade_down'] = (df['drop_pct'] < -0.004) & df['vol_spike']
    
    # Cascade up: High to Open pump > 0.4% in 1 minute
    df['pump_pct'] = (df['high'] - df['open']) / df['open']
    df['cascade_up'] = (df['pump_pct'] > 0.004) & df['vol_spike']
    ...
    for i in range(60, n - 60):
        if not in_position:
            if i > cooldown_until:
                if signals_down[i]:
                    # Fade the dump (Limit BUY 0.15% below the close)
                    ...
                    # Fade the pump (Limit SELL 0.15% above the close)
                    limit_price = closes[i] * (1 + 0.0015)
                    filled = False
                    for j in range(1, 4):
            ...
            if pos_type == 1: # LONG
                target_price = entry_price * (1 + 0.0015) # 0.15% TP
...
def main():
    ...
    print(f"Step 1/2: Simulating Config 2 (No SL, Limit Entries) on {len(symbols)} Top Coins...")
    ...
    print("\n" + "="*50)
    print("--- Strategy Results (Perfected Microstructure Fade) ---")
    print("="*50)
