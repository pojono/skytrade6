#!/usr/bin/env python3
"""
v42bh: Garman-Klass Vol + Price Efficiency Ratio + Return Concentration + Tick Intensity

EXP LLLL6: Garman-Klass Volatility
  - GK = sqrt(0.5*ln(H/L)^2 - (2*ln2-1)*ln(C/O)^2)
  - More efficient than Parkinson, uses OHLC
  - Extreme GK vol → fade

EXP MMMM6: Price Efficiency Ratio (Kaufman-like)
  - ER = abs(close - close[N]) / sum(abs(close[i] - close[i-1]))
  - High ER = trending efficiently → fade exhaustion
  - Low ER = choppy → MR

EXP NNNN6: Return Concentration (Herfindahl-like)
  - Square each bar's return share of total absolute returns
  - High concentration = few bars dominate = spiky
  - Low concentration = distributed = smooth

EXP OOOO6: Tick Intensity (bars with extreme range / total bars)
  - Fraction of bars in window with range > 2x median range
  - High intensity = volatile period → fade
...
def main():
    out_file = 'results/v42bh_gk_efficiency.txt'
    ...
    print("="*80)
    print(f"  v42bh: GARMAN-KLASS VOL + EFFICIENCY RATIO + RETURN CONC + TICK INTENSITY")
    ...
    for symbol in symbols:
        ...
        log_co = np.log(close / (opn + 1e-10))
        gk_component = 0.5 * log_hl**2 - (2*np.log(2) - 1) * log_co**2

        for window in [30, 60]:
            gk_vol = np.sqrt(gk_component.rolling(window, min_periods=window).mean().clip(lower=0))
            gk_pct = gk_vol.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.90, 0.95]:
                high_gk = gk_pct > thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[high_gk]:
                ...
                print(f"\n  GK vol({window}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'gk_{window}_{thresh}')] = te_r

        # ── EXP MMMM6: PRICE EFFICIENCY RATIO ──
        print(f"\n  --- EXP MMMM6: PRICE EFFICIENCY RATIO ---")

        for window in [20, 40]:
            net_move = (close - close.shift(window)).abs()
            path = ret_1m.abs().rolling(window, min_periods=window).sum() * close
            eff_ratio = net_move / (path + 1e-10)
            er_pct = eff_ratio.rolling(120, min_periods=60).rank(pct=True)

            # High ER = trending → fade
            for thresh in [0.90, 0.95]:
                high_er = er_pct > thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[high_er]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                ...
                print(f"\n  Eff ratio({window}) >{thresh*100:.0f}th pct (fade trend):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'er_{window}_{thresh}')] = te_r

        # ── EXP NNNN6: RETURN CONCENTRATION ──
        print(f"\n  --- EXP NNNN6: RETURN CONCENTRATION ---")
        ...
        for window in [20, 40]:
            ...
            ret_conc = abs_ret.rolling(window, min_periods=window).apply(herfindahl, raw=True)
            rc_pct = ret_conc.rolling(120, min_periods=60).rank(pct=True)

            # High concentration = spiky = overextended → fade
            for thresh in [0.90, 0.95]:
                high_conc = rc_pct > thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[high_conc]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    r = ret_1m.iloc[idx] if idx < len(ret_1m) else 0
                    is_long = r < 0
                ...
                print(f"\n  Ret conc({window}) >{thresh*100:.0f}th pct (fade spiky):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'rc_{window}_{thresh}')] = te_r

        # ── EXP OOOO6: TICK INTENSITY ──
        print(f"\n  --- EXP OOOO6: TICK INTENSITY ---")

        bar_range = high - low

        for window in [30, 60]:
            med_range = bar_range.rolling(window, min_periods=window).median()
            extreme_bar = (bar_range > 2 * med_range).astype(float)
            tick_int = extreme_bar.rolling(window, min_periods=window).mean()
            ti_pct = tick_int.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.90, 0.95]:
                high_ti = ti_pct > thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[high_ti]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    r = ret_1m.iloc[idx] if idx < len(ret_1m) else 0
                    is_long = r < 0
                ...
                print(f"\n  Tick int({window}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'ti_{window}_{thresh}')] = te_r
