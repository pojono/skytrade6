...
def load_data(symbol, start_date="2025-01-01"):
    try:
        ...
        # Merge and Resample to 5-minute to give us better timing than 1-hour
...
def generate_signals(df):
    # 24-hour rolling window (288 periods for 5-min data) for Z-scores
    ...
    # Let's add a momentum filter to avoid fighting a massive trend
    # 6-hour momentum
    ...
    # Filter: Momentum must not be insanely positive (don't short a vertical green candle)
    df.loc[(df['count_z'] > 2.0) & (df['vol_z'] < -2.0) & (df['mom_6h'] < 0.05), 'signal'] = -1
    
    # Bullish Divergence (Retail Short, Whales Long) -> Long
    # Filter: Momentum must not be insanely negative (don't catch a falling knife)
    df.loc[(df['count_z'] < -2.0) & (df['vol_z'] > 2.0) & (df['mom_6h'] > -0.05), 'signal'] = 1
...
def run_strat_2(symbol):
    ...
    # Hold for 4 hours (48 periods, since it's 5m data)
    # Taker fee 5 bps
    results = run_backtest(df, hold_periods=48, fee_bps=5.0)
...
if __name__ == "__main__":
    ...
    print("--- Backtesting Strategy 2: The Whale Shadow V2 (Tighter triggers, Mom Filter) ---")
