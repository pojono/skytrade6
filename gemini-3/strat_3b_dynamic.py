...
def run_strat_3_dynamic(symbol):
    df = load_data(symbol)
    if df is None or len(df) < 500: return None
    
    # Needs high/low for accurate intra-hour TP/SL
    ...
    # Merge high/low back into hourly for TP/SL engine
    df = df.join(hl_df.resample('1h').agg({'high': 'max', 'low': 'min'}), how='left')
    df['high'] = df['high'].fillna(df['close'])
    df['low'] = df['low'].fillna(df['close'])
    
    # Test a realistic TP/SL based on historical average liquidation wick size
    results = run_backtest_tp_sl(df, tp_pct=0.08, sl_pct=0.03, fee_bps=5.0)
...
if __name__ == "__main__":
    ...
    print("--- Backtesting Strategy 3: Powder Keg with Dynamic TP/SL (8% TP / 3% SL) ---")
    
    res_list = []
    with Pool(min(4, os.cpu_count() or 4)) as p:
        for r in p.imap_unordered(run_strat_3_dynamic, symbols):
