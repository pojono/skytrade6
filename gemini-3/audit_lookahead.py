...
def audit_single_trade(symbol="BTCUSDT"):
    ...
    dfs = []
    for f in kline_files[:30]: # Just need a few months for an audit trade
    ...
    # Isolate the first trade
    trades = hourly[hourly['signal'] != 0].copy()
    if len(trades) == 0:
        print("No trades found in this slice to audit.")
        return
    ...
    # --- AUDIT POINT 1: SIGNAL GENERATION LEAKAGE ---
    # To get a Z-score at 08:00:00, we should only use data up to 08:00:00.
    # The '.rolling().mean()' function is right-aligned by default in pandas, meaning the value at index T includes T.
    # Because we resampled to '1h', the row '08:00:00' represents data from 07:01 to 08:00.
    # The 'last' aggregate means the 'close' at 08:00:00 is the exact close of the 07:59 minute candle (which closes exactly at 08:00:00).
    print("\n--- Audit Point 1: Execution Price Alignment ---")
    print(f"Hourly Signal Row Time: {first_trade_time}")
    print(f"Hourly Close Price (Assumed Entry): {first_trade['close']}")
    
    # Verify against raw 1m data
    ...
    if abs(first_trade['close'] - raw_1m_at_execution['close']) < 0.01:
        print("✅ PASS: The hourly close perfectly matches the raw 1-minute close preceding the signal.")
    else:
        print("❌ FAIL: Price mismatch. Lookahead bias detected in resampling.")
        
    # --- AUDIT POINT 2: EXECUTION PATH LEAKAGE ---
    ...
    print(f"Execution Path Start: {path.index[0]}")
    print(f"Execution Path End: {path.index[-1]}")
    
    if path.index[0] > first_trade_time:
        print("✅ PASS: The execution path strictly begins AFTER the signal generation time.")
    else:
