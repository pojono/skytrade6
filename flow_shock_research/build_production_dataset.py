#!/usr/bin/env python3
"""
Build production event dataset with orderbook.500 data.

Using July-Aug 2025 sample where orderbook.500 is available.
Extract all features needed for production system.
"""
import pandas as pd
import numpy as np
            ...
            for line in f:
                try:
                    ...
                    if 'result' in data and 'data' in data['result']:
                        ...
                        ts = int(data['result']['ts'])  # Timestamp is in result.ts
...
def extract_orderbook_features(ob_snapshot):
    ...
    try:
        ...
        print(f"Error extracting OB features: {e}", flush=True)
        return {}
...
print("="*80, flush=True)
print("📦 BUILDING PRODUCTION DATASET", flush=True)
print("="*80, flush=True)
print("\nUsing July-Aug 2025 sample with orderbook.500 data", flush=True)
print("="*80 + "\n", flush=True)
...
for idx, row in events_df.iterrows():
    if (idx + 1) % 20 == 0:
        print(f"   Processing {idx + 1}/{len(events_df)}...", flush=True)
