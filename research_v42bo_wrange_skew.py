#!/usr/bin/env python3
"""
v42bo: Weighted Range + CLV Persistence + Return Skewness + Price Oscillation Amp

EXP NNNN7: Weighted Range (range weighted by close position)
  - wrange = range * abs(2*(close-low)/range - 1)
  - Captures range with directional conviction
  - Extreme weighted range → fade

EXP OOOO7: CLV Persistence (close location value trend)
  - CLV = (2*close - high - low) / (high - low)
  - Rolling sum of CLV = persistent buying/selling pressure
  - Extreme CLV persistence → fade

EXP PPPP7: Return Skewness (asymmetry of return distribution)
  - Rolling skewness of 1-min returns
  - Positive skew = right tail, negative = left tail
  - Extreme skew → fade

EXP QQQQ7: Price Oscillation Amplitude
  - Detrended price: residual from rolling mean
  - Amplitude = rolling std of detrended price / price
  - High amplitude = choppy → MR opportunity → fade
...
def main():
    out_file = 'results/v42bo_wrange_skew.txt'
    ...
    print("="*80)
    print(f"  v42bo: WEIGHTED RANGE + CLV PERSIST + RET SKEWNESS + OSC AMPLITUDE")
    ...
    for symbol in symbols:
        ...
        # ── EXP NNNN7: WEIGHTED RANGE ──
        print(f"\n  --- EXP NNNN7: WEIGHTED RANGE ---")

        clv = (2 * close - high - low) / (bar_range + 1e-10)
        wrange = bar_range * clv.abs()

        for window in [20, 40]:
            wr_avg = wrange.rolling(window, min_periods=window).mean()
            ...
            for thresh in [0.90, 0.95]:
                ...
                print(f"\n  Wt range({window}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'wr_{window}_{thresh}')] = te_r

        # ── EXP OOOO7: CLV PERSISTENCE ──
        print(f"\n  --- EXP OOOO7: CLV PERSISTENCE ---")

        for window in [20, 40]:
            clv_sum = clv.rolling(window, min_periods=window).sum()
            cs_pct = clv_sum.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.05, 0.10]:
                up_ext = cs_pct > (1 - thresh)
                dn_ext = cs_pct < thresh
                ...
                print(f"\n  CLV persist({window}) <{thresh}/{1-thresh:.2f} (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'clvp_{window}_{thresh}')] = te_r

        # ── EXP PPPP7: RETURN SKEWNESS ──
        print(f"\n  --- EXP PPPP7: RETURN SKEWNESS ---")
        ...
        for window in [30, 60]:
            ...
            ret_skew = ret_1m.rolling(window, min_periods=window).apply(skew_func, raw=True)
            sk_pct = ret_skew.abs().rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.90, 0.95]:
                extreme_sk = sk_pct > thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[extreme_sk]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    sk_val = ret_skew.iloc[idx]
                    is_long = sk_val < 0  # fade skew direction
                    t = sim_trade(bars, idx, is_long)
                    if t:
                        if ts < split_ts: train_trades.append(t)
                        else: test_trades.append(t)
                ...
                print(f"\n  Ret skew({window}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'sk_{window}_{thresh}')] = te_r
        ...
        for window in [30, 60]:
            ...
            detrended = close - roll_mean
            osc_amp = detrended.rolling(window, min_periods=window).std() / (close + 1e-10)
            oa_pct = osc_amp.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.90, 0.95]:
                high_oa = oa_pct > thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[high_oa]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    r = ret_1m.iloc[idx] if idx < len(ret_1m) else 0
                    is_long = r < 0
                ...
                print(f"\n  Osc amp({window}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'oa_{window}_{thresh}')] = te_r
