#!/usr/bin/env python3
"""
v42bn: Price Gap Ratio + Return Sign Persist + HL Momentum + Candle Efficiency

EXP JJJJ7: Price Gap Ratio (gap size relative to range)
  - gap = |open - prev_close| / prev_range
  - Rolling average of gap ratio
  - High gap ratio = gappy/illiquid → fade

EXP KKKK7: Return Sign Persistence (fraction of same-sign returns)
  - Count fraction of positive (or negative) returns in window
  - Extreme fraction = one-sided market → fade

EXP LLLL7: High-Low Momentum (momentum using highs and lows)
  - hl_mom = (high - high.shift(N)) + (low - low.shift(N))
  - Captures range expansion direction
  - Extreme HL momentum → fade

EXP MMMM7: Candle Efficiency (net move / total path)
  - efficiency = |close - open| / (high - low)
  - Low efficiency = indecision, high efficiency = conviction
  - Rolling std of efficiency = regime instability → fade
...
def main():
    out_file = 'results/v42bn_gapret_hlmom.txt'
    ...
    print("="*80)
    print(f"  v42bn: GAP RATIO + RET SIGN PERSIST + HL MOMENTUM + CANDLE EFFICIENCY")
    ...
    for symbol in symbols:
        ...
        # ── EXP JJJJ7: PRICE GAP RATIO ──
        print(f"\n  --- EXP JJJJ7: PRICE GAP RATIO ---")

        gap_abs = (opn - close.shift(1)).abs()
        prev_range = bar_range.shift(1)
        gap_ratio = gap_abs / (prev_range + 1e-10)

        for window in [20, 40]:
            gr_avg = gap_ratio.rolling(window, min_periods=window).mean()
            gr_pct = gr_avg.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.90, 0.95]:
                high_gr = gr_pct > thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[high_gr]:
                ...
                print(f"\n  Gap ratio({window}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'gr_{window}_{thresh}')] = te_r

        # ── EXP KKKK7: RETURN SIGN PERSISTENCE ──
        print(f"\n  --- EXP KKKK7: RETURN SIGN PERSISTENCE ---")

        pos_sign = (ret_1m > 0).astype(float)

        for window in [20, 40]:
            sign_frac = pos_sign.rolling(window, min_periods=window).mean()
            sf_pct = sign_frac.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.05, 0.10]:
                up_ext = sf_pct > (1 - thresh)
                dn_ext = sf_pct < thresh
                ...
                print(f"\n  Ret sign({window}) <{thresh}/{1-thresh:.2f} (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'rs_{window}_{thresh}')] = te_r

        # ── EXP LLLL7: HIGH-LOW MOMENTUM ──
        print(f"\n  --- EXP LLLL7: HIGH-LOW MOMENTUM ---")

        for window in [20, 40]:
            hl_mom = (high - high.shift(window)) + (low - low.shift(window))
            hl_norm = hl_mom / (close + 1e-10)
            hlm_pct = hl_norm.abs().rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.90, 0.95]:
                extreme_hlm = hlm_pct > thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[extreme_hlm]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    is_long = hl_norm.iloc[idx] < 0
                    t = sim_trade(bars, idx, is_long)
                    if t:
                        if ts < split_ts: train_trades.append(t)
                        else: test_trades.append(t)
                ...
                print(f"\n  HL mom({window}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'hlm_{window}_{thresh}')] = te_r

        # ── EXP MMMM7: CANDLE EFFICIENCY STD ──
        print(f"\n  --- EXP MMMM7: CANDLE EFFICIENCY STD ---")

        candle_eff = (close - opn).abs() / (bar_range + 1e-10)

        for window in [20, 40]:
            ce_std = candle_eff.rolling(window, min_periods=window).std()
            ces_pct = ce_std.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.90, 0.95]:
                high_ces = ces_pct > thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[high_ces]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    r = ret_1m.iloc[idx] if idx < len(ret_1m) else 0
                    is_long = r < 0
                ...
                print(f"\n  Candle eff std({window}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'ces_{window}_{thresh}')] = te_r
