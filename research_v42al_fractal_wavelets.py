#!/usr/bin/env python3
"""
v42al: Fractal Dimension + Wavelet-Inspired Signals

EXP BBBB2: Fractal Dimension (Higuchi)
  - Approximate fractal dimension of price series
  - High FD → choppy/random → fade
  - Low FD → smooth/trending → follow

EXP CCCC2: Multi-Scale Momentum
  - Compare momentum at 5, 15, 30, 60 min scales
  - When all scales agree → strong signal
  - When scales disagree → regime change

EXP DDDD2: Z-Score of Z-Scores
  - Meta-signal: z-score of multiple z-scores
  - Captures when MULTIPLE indicators are extreme simultaneously
  - Higher-order confirmation

EXP EEEE2: Relative Strength vs Moving Average
  - Price distance from multiple MAs (5, 20, 60)
  - When all MAs agree on direction → strong signal
  - When price is extreme vs all MAs → fade
...
def main():
    out_file = 'results/v42al_fractal_wavelets.txt'
    ...
    print("="*80)
    print(f"  v42al: FRACTAL + MULTI-SCALE + META-SIGNALS")
    ...
    for symbol in symbols:
        ...
        mom15 = close.pct_change(15)
        mom30 = close.pct_change(30)
        mom60 = close.pct_change(60)

        # All scales agree on direction → strong signal, fade it
        all_up = (mom5 > 0) & (mom15 > 0) & (mom30 > 0) & (mom60 > 0)
        all_down = (mom5 < 0) & (mom15 < 0) & (mom30 < 0) & (mom60 < 0)

        train_trades = []; test_trades = []
        lt = None
        for ts in bars.index[all_up]:
            if lt and (ts - lt).total_seconds() < 60: continue
            idx = bars.index.get_loc(ts)
            t = sim_trade(bars, idx, False)  # fade all-up
            if t:
                if ts < split_ts: train_trades.append(t)
                else: test_trades.append(t)
                lt = ts
        for ts in bars.index[all_down]:
            if lt and (ts - lt).total_seconds() < 60: continue
            idx = bars.index.get_loc(ts)
            t = sim_trade(bars, idx, True)  # fade all-down
            if t:
                if ts < split_ts: train_trades.append(t)
                else: test_trades.append(t)
                lt = ts
        ...
        z_ret = ret_1m / (roll_std + 1e-10)

        rsi = compute_rsi(close, 14)
        rsi_z = (rsi - 50) / 15  # normalize RSI to z-score-like

        ema10 = close.ewm(span=10, adjust=False).mean()
        ema50 = close.ewm(span=50, adjust=False).mean()
        ema_div = (ema10 - ema50) / ema50 * 10000
        ema_std = ema_div.rolling(60, min_periods=30).std()
        z_ema = ema_div / (ema_std + 1e-10)

        low_k = bars['low'].rolling(30, min_periods=30).min()
        high_k = bars['high'].rolling(30, min_periods=30).max()
        stoch = (close - low_k) / (high_k - low_k + 1e-10) * 100
        stoch_z = (stoch - 50) / 25

        # Meta z-score: average of individual z-scores
        meta_z = (z_ret + rsi_z + z_ema + stoch_z) / 4

        for z_thresh in [1.5, 2]:
            extreme_up = meta_z > z_thresh
            extreme_down = meta_z < -z_thresh
            ...
            for ts in bars.index[extreme_up]:
                if lt and (ts - lt).total_seconds() < 60: continue
                idx = bars.index.get_loc(ts)
                t = sim_trade(bars, idx, False)  # fade extreme up meta-z
                if t:
                    if ts < split_ts: train_trades.append(t)
                    else: test_trades.append(t)
                    lt = ts
            ...
            print(f"\n  Meta-Z |>{z_thresh}| (fade):")
            ts_r = pstats(train_trades, "TRAIN")
            te_r = pstats(test_trades, "TEST")
            if ts_r and te_r:
                oos = "✅" if te_r['tot'] > 0 else "❌"
                print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                grand[(symbol, f'meta_z_{z_thresh}')] = te_r

        # ── EXP EEEE2: RELATIVE STRENGTH VS MULTIPLE MAs ──
        print(f"\n  --- EXP EEEE2: PRICE VS MULTIPLE MAs ---")

        ma5 = close.rolling(5, min_periods=5).mean()
        ma20 = close.rolling(20, min_periods=20).mean()
        ma60 = close.rolling(60, min_periods=60).mean()

        dist5 = (close - ma5) / ma5 * 10000
        dist20 = (close - ma20) / ma20 * 10000
        dist60 = (close - ma60) / ma60 * 10000

        # All MAs agree: price above all → overbought → short
        all_above = (dist5 > 0) & (dist20 > 0) & (dist60 > 0)
        all_below = (dist5 < 0) & (dist20 < 0) & (dist60 < 0)

        # Combined distance
        total_dist = dist5 + dist20 + dist60
        dist_std = total_dist.rolling(60, min_periods=30).std()

        for z_thresh in [2, 3]:
            extreme_above = all_above & (total_dist > z_thresh * dist_std)
            extreme_below = all_below & (total_dist < -z_thresh * dist_std)

            train_trades = []; test_trades = []
            lt = None
            for ts in bars.index[extreme_above]:
                if lt and (ts - lt).total_seconds() < 60: continue
                idx = bars.index.get_loc(ts)
                ...
                if t:
                    ...
                    lt = ts
            for ts in bars.index[extreme_below]:
                if lt and (ts - lt).total_seconds() < 60: continue
                idx = bars.index.get_loc(ts)
                t = sim_trade(bars, idx, True)
                if t:
            ...
            print(f"\n  All MAs agree + dist z>{z_thresh} (fade):")
            ts_r = pstats(train_trades, "TRAIN")
            te_r = pstats(test_trades, "TEST")
            if ts_r and te_r:
                oos = "✅" if te_r['tot'] > 0 else "❌"
                print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                grand[(symbol, f'ma_dist_{z_thresh}')] = te_r
