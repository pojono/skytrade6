#!/usr/bin/env python3
"""
v42bc: VWAP Deviation + Rolling HL Correlation + MR Speed + Volatility Persistence

EXP RRRR5: VWAP Deviation (price vs volume-weighted avg)
  - Proxy VWAP using typical price * volume proxy (range as vol proxy)
  - Extreme deviation from VWAP → fade back to VWAP

EXP SSSS5: Rolling High-Low Correlation
  - Correlation between high and low over N bars
  - High corr = tight range = squeeze → breakout
  - Low corr = wide range = volatile → MR

EXP TTTT5: Mean Reversion Speed
  - How fast does price return to SMA after deviation?
  - Fast MR speed = strong MR regime → fade with confidence
  - Slow MR speed = trending → avoid MR

EXP UUUU5: Volatility Persistence (GARCH-like)
  - Autocorrelation of absolute returns
  - High persistence = vol clustering = more predictable
  - Extreme vol persistence + recent move → fade
...
def main():
    out_file = 'results/v42bc_vwap_corr_mrspeed.txt'
    ...
    print("="*80)
    print(f"  v42bc: VWAP DEV + HL CORR + MR SPEED + VOL PERSISTENCE")
    ...
    for symbol in symbols:
        ...
        # ── EXP RRRR5: VWAP DEVIATION ──
        print(f"\n  --- EXP RRRR5: VWAP DEVIATION ---")

        typical = (high + low + close) / 3
        vol_proxy = high - low  # range as volume proxy

        for window in [30, 60]:
            vwap = (typical * vol_proxy).rolling(window, min_periods=window).sum() / \
                   (vol_proxy.rolling(window, min_periods=window).sum() + 1e-10)
            vwap_dev = (close - vwap) / (vwap + 1e-10) * 10000  # bps
            vwap_pct = vwap_dev.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.05, 0.10]:
                above = vwap_pct > (1 - thresh)
                below = vwap_pct < thresh
                ...
                print(f"\n  VWAP dev({window}) <{thresh}/{1-thresh:.2f} (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'vwap_{window}_{thresh}')] = te_r

        # ── EXP SSSS5: ROLLING HL CORRELATION ──
        print(f"\n  --- EXP SSSS5: ROLLING HL CORRELATION ---")

        for window in [30, 60]:
            hl_corr = high.rolling(window, min_periods=window).corr(low)
            hc_pct = hl_corr.rolling(120, min_periods=60).rank(pct=True)

            # Low corr = wide range = volatile → fade
            for thresh in [0.10, 0.20]:
                low_corr = hc_pct < thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[low_corr]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                ...
                print(f"\n  HL corr({window}) <{thresh*100:.0f}th pct (fade vol):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'hlcorr_{window}_{thresh}')] = te_r

        # ── EXP TTTT5: MEAN REVERSION SPEED ──
        print(f"\n  --- EXP TTTT5: MEAN REVERSION SPEED ---")

        for sma_window in [20, 40]:
            sma = close.rolling(sma_window, min_periods=sma_window).mean()
            dev = (close - sma) / (sma + 1e-10)
            # MR speed = how fast deviation shrinks
            dev_change = dev.diff().abs()
            mr_speed = dev_change.rolling(30, min_periods=20).mean()
            ms_pct = mr_speed.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.90, 0.95]:
                fast_mr = ms_pct > thresh

                train_trades = []; test_trades = []
                lt = None
                for ts in bars.index[fast_mr]:
                    if lt and (ts - lt).total_seconds() < 60: continue
                    idx = bars.index.get_loc(ts)
                    r = ret_1m.iloc[idx] if idx < len(ret_1m) else 0
                    is_long = r < 0
                ...
                print(f"\n  MR speed({sma_window}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'mrspd_{sma_window}_{thresh}')] = te_r

        # ── EXP UUUU5: VOLATILITY PERSISTENCE ──
        print(f"\n  --- EXP UUUU5: VOLATILITY PERSISTENCE ---")

        abs_ret = ret_1m.abs()

        for window in [30, 60]:
            vol_ac = abs_ret.rolling(window, min_periods=window).apply(
                lambda x: pd.Series(x).autocorr(lag=1), raw=False)
            vac_pct = vol_ac.rolling(120, min_periods=60).rank(pct=True)

            for thresh in [0.90, 0.95]:
                high_persist = vac_pct > thresh
                ...
                print(f"\n  Vol persist({window}) >{thresh*100:.0f}th pct (fade):")
                ts_r = pstats(train_trades, "TRAIN")
                te_r = pstats(test_trades, "TEST")
                if ts_r and te_r:
                    oos = "✅" if te_r['tot'] > 0 else "❌"
                    print(f"    {oos} test={te_r['tot']/28:+.3f}%/d")
                    grand[(symbol, f'volp_{window}_{thresh}')] = te_r
